...
type node struct {
	...
	left, right []byte    // this is where the two children
...
// using Sum presupposes sequantial synchronous writes (io.Writer interface)
...
// AsyncWriter is an asynchronous segment/section writer interface for BMT hash
// AsyncWriter is unsafe and does not check indexes and section data lengths
...
// * set length in Hash call when length/secsize < maxsec
//
...
//   and keeps the resource
//   can be released calling Reset()
...
// Hash can be called any time once the length and the span is known
// potentially even before all segments have been written
// in such cases Hash will block until all segments are present and
// the hash for the length can be calculated.
//
// b: digest is appended to b
// length: known legnth of the input (unsafe; undefined if out of range)
